...
<div class="container">
  	<div class="panel panel-default">
		...
		<div class="panel-heading">Data Order Test Lab </div>
		...
		<table class="table table-stripped table-bordered">
			...
			@foreach ($orders as $o => $val)
				<?php $otr = ''; ?>
				<?php $otr = explode("#", $val->other); //print "<pre>"; print_r($otr); ?>
				<tr>
					...
					<td>{!! $val->status !!} @if( $val->status == 0) Pending @elseif( $val->status == 1) On Process @elseif( $val->status == 3) Hasil Sudah Diemail @else Cancel @endif</td>
					<td>{!! $val->nama_lab !!}</td>
					<td class="text-center">
						<div class="btn-group">
							<a href="show/{{ $val->orders_code }}" class="btn btn-sm btn-default" title="View" data-toggle="tooltip"><i class="glyphicon glyphicon-eye-open"></i></a>
						</div>
					</td>
				</tr>
